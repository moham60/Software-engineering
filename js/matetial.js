let uploadMaterial = document.getElementById("uploadMaterial");
let addMaterial = document.getElementById("addMaterial");
let materialRow = document.querySelector(".materialRow");
var countPng = 0,
  countpdf = 0,
  countppt = 0;

addMaterial.addEventListener("click", function () {
  if (uploadMaterial.value !== "") {
    var extension = uploadMaterial.files[0].name.slice(
      uploadMaterial.files[0].name.lastIndexOf(".") + 1
    );
    if (
      extension === "pdf" ||
      extension === "png" ||
    ...
    <img width="55p" src="./images/${
      extension === "pptx" ? "eps" : extension
    }.svg" alt="">           
      <span class="d-block mt-2 num-assignment">${
        uploadMaterial.files[0].name
      } </span>
      <div class="info mt-2 border-top pt-3 d-flex align-items-center justify-content-between   ">
          
      <span class="text-secondary">${(
        uploadMaterial.files[0].size / 1000000
      ).toPrecision(2)}mb</span>
      </div>
          </div>
          </div>`;
    } else {
    ...
    if (extension === "png") {
      ...
      document.querySelector(
        ".images-files"
      ).innerHTML = `<div class="d-flex w-100 align-items-center justify-content-between">
